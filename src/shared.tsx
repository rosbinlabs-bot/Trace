...
// Project Direct Cost is a recurring MONTHLY figure, same basis as Monthly Fee (entered once on
// Project Master, but applies every month like the fee does) -- so margin is Monthly Fee minus
// Direct Cost, both monthly, not Direct Cost against the whole-project Total Value. Replaces the
// old `margin` DB column, which was never exposed in any edit UI and always read 0/empty for real
// projects (see Dashboard.tsx's on-time-delivery comment for the same history) -- this is computed
// live instead, the same fix Dashboard.tsx already applied elsewhere.
export const projMarginPct = (p) => {
  const fee = Number(p.monthlyFee)||0;
  if (!fee) return 0;
  return Math.round(((fee - (Number(p.directCost)||0)) / fee) * 100);
};
...
// scope="col" is the one-line fix that lets a screen reader announce "Description, column" (etc.)
// when it lands on an editable cell in that column instead of just "edit text" with no context --
// benefits every table in the app that uses S.Th (nearly all of them) in a single place, rather than
// hand-adding an aria-label to every individual inline-editable input (Changes.tsx, Team.tsx, etc.).
export const Th = ({children}: any) => <th scope="col" className="text-left px-3 py-2 font-medium text-slate-500 text-xs uppercase tracking-wide">{children}</th>;
...
/* ============================ REPORTS: charts ============================
   The recharts-dependent chart wrapper components (BarChartMini, HBarChartMini, LineChartMini,
   DonutChartMini, ChartBlock, CHART_COLORS) that used to live here moved to `chartWidgets.tsx`
   (2026-08-24, perf pass). shared.tsx is imported eagerly by App.tsx on every page load; recharts
   is a large library only actually needed by the Reports screen, so keeping it out of shared.tsx
   keeps it out of the eager bundle. Import from '../chartWidgets' (screens) instead. */

// ---- month-bucket helpers, used by every trend chart above ----
// Ascending list of the last `n` months (oldest first, current month last), as both a 'YYYY-MM' key
// (for matching against stored dates) and a short display label (e.g. 'Jan 26').
export const lastNMonths = (n: number): {key:string,label:string}[] => {
  const out: {key:string,label:string}[] = [];
  const base = new Date(TODAY_ISO+'T00:00:00');
  for (let i=n-1; i>=0; i--) {
    const dt = new Date(base.getFullYear(), base.getMonth()-i, 1);
    const key = `${dt.getFullYear()}-${String(dt.getMonth()+1).padStart(2,'0')}`;
    out.push({ key, label: dt.toLocaleDateString('en-US',{month:'short', year:'2-digit'}) });
  }
  return out;
};
// How many `items` have `dateField` falling in each of the last-N-months buckets.
export const countByMonth = (items: any[], dateField: string, months: {key:string,label:string}[]) => {
  const counts: Record<string, number> = {};
  months.forEach(m=>counts[m.key]=0);
  (items||[]).forEach(it=>{ const k=String(it[dateField]||'').slice(0,7); if (k in counts) counts[k]++; });
  return months.map(m=>({ month: m.label, count: counts[m.key] }));
};
// Sum of `amountField` for items (optionally filtered by `filterFn`) whose `dateField` falls in each
// month bucket -- the revenue-trend chart's data source.
export const sumByMonth = (items: any[], dateField: string, amountField: string, months: {key:string,label:string}[], filterFn?: (it:any)=>boolean) => {
  const sums: Record<string, number> = {};
  months.forEach(m=>sums[m.key]=0);
  (items||[]).filter(it=>!filterFn || filterFn(it)).forEach(it=>{ const k=String(it[dateField]||'').slice(0,7); if (k in sums) sums[k]+=(Number(it[amountField])||0); });
  return months.map(m=>({ month: m.label, amount: sums[m.key] }));
};
...
export const REPORT_CATALOG: any = {
  'Executive Reports': [
    ...
    { key:'revenuetrend', label:'Revenue Trend (12 Months)' },
    { key:'riskdash', label:'Risk Dashboard' },
    { key:'margin', label:'Margin Analysis' },
    { key:'billingsummary', label:'Billing & Payment Status' },
    { key:'billingaging', label:'Collections Aging' },
    { key:'portfoliomix', label:'Portfolio Mix' },
  ],
  'Project Reports': [
    ...
    { key:'velocity', label:'Delivery Velocity (12 Months)' },
    { key:'riskissuetrend', label:'Risk & Issue Trend (12 Months)' },
  ],
  'Team Reports': [
    ...
    { key:'utildist', label:'Utilization Distribution' },
  ],
  'Client Reports': [
    ...
    { key:'clientrevenue', label:'Client Revenue Contribution' },
    { key:'clientriskheat', label:'Client Risk Heat' },
    { key:'clientmix', label:'Industry & Engagement Mix' },
  ],
};
