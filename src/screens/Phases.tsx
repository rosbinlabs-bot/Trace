...
export default function Phases(){
  ...
  // Once a deadline has been entered on a phase, milestone or sub task, only Admin/Super Admin
  // (role==='admin') may change it -- otherwise any tagged team member could quietly slide a
  ...
  // reviewSince stamps the day the Implemented escalation first queued too (same as the ordinary
  // Completed -> Pending Review path in S.applyStatus) -- preserved with ||, never overwritten, so
  // the sidebar/Approvals "stuck >= S.STUCK_APPROVAL_DAYS days" logic measures from when it actually
  // started, not from whichever chain hop happens to be current.
  const markImplementedMs = (phId, msId) => { mutMs(phId, msId, m => { const chain = S.implementChainFor(projMeta, actor); return {...m, review:'Implemented Review', implChain:chain, implApprovals:[], headApprovedImpl:chain.length===0, clientApprovedImpl:false, reviewSince:m.reviewSince||S.TODAY_ISO}; }); const ph=phases.find(p=>p.id===phId); const ms=ph?.milestones?.find(m=>m.id===msId); logPhase(`Started Implemented escalation for milestone "${ms?.name||''}"`); };
  const markImplementedSt = (phId, msId, stId) => { mutSt(phId, msId, stId, s => { const chain = S.implementChainFor(projMeta, actor); return {...s, review:'Implemented Review', implChain:chain, implApprovals:[], headApprovedImpl:chain.length===0, clientApprovedImpl:false, reviewSince:s.reviewSince||S.TODAY_ISO}; }); const ph=phases.find(p=>p.id===phId); const st=ph?.milestones?.find(m=>m.id===msId)?.subtasks?.find(s=>s.id===stId); logPhase(`Started Implemented escalation for sub task "${st?.name||''}"`); };
