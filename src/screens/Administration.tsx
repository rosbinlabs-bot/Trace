  ...
  return (
    <div>
      ...
      <div className="space-y-2">
        {messages.map((m: any) => {
          const expired = S.flashExpired(m);
          return (
          <S.Card key={m.id} className={`p-3 ${(m.active===false || expired) ? 'opacity-60' : ''}`}>
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex items-center gap-1.5 shrink-0 flex-wrap justify-end">
                ...
                <S.Badge cls="bg-violet-50 text-violet-700">{flashExpiryLabel(m)}</S.Badge>
                {expired
                  ? <S.Badge cls="bg-slate-100 text-slate-500">Expired</S.Badge>
