...
export default function Approvals(){
  ...
  const headPending = allEntries.filter(e=>e.item.review==='Head Review' || (e.item.review==='Implemented Review' && !e.item.headApprovedImpl));
  ...
  const stages = [
    { label:'PM Verification', count: pmPending.length },
    { label:'Head Review', count: headPending.length },
    { label:'Client Sign-off', count: clientPending.length },
    { label:'Implemented', count: implemented.length },
  ];
  const pending = [
    ...pmPending.map(e=>({...e, stage:'PM Verification'})),
    ...headPending.map(e=>({...e, stage:'Head Review'})),
    ...clientPending.map(e=>({...e, stage:'Client Sign-off'})),
  ];

  return (
    <div>
      <S.SectionTitle sub="Live pending approvals across all projects — act on these from Phase Management (PM/Head) or Client Portal (Client)">Client Approval Workflow</S.SectionTitle>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <S.Card className="p-4">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between"><span className="text-slate-500">Awaiting PM Verification</span><span className="font-medium text-amber-600">{pmPending.length}</span></div>
            <div className="flex justify-between"><span className="text-slate-500">Awaiting Head Review</span><span className="font-medium text-amber-600">{headPending.length}</span></div>
          ...
          <div className="text-xs text-slate-400 mt-4 pt-3 border-t border-slate-100">To approve or reject an item, open it in Phase Management (PM/Project Head) or Client Portal (Client Owner) — decisions made there are reflected here immediately.</div>
        </S.Card>
      </div>
    </div>
  );
