          ...
          <tbody className="divide-y divide-slate-100">
            {changes.map(c=>(
              <tr key={c.id} className="hover:bg-slate-50">
                ...
                <S.Td><select aria-label="Status" className={`${S.gInp} inline-block w-auto`} value={c.status} onChange={e=>mut(c.id,{status:e.target.value})}>{['Pending','Approved','Rejected','Implemented'].map(o=><option key={o}>{o}</option>)}</select></S.Td>
