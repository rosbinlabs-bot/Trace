...
// L2=Project Head, L3=PM, L4=Associate, the same default mapping Administration -> Roles &
// Permissions seeds) so nothing already assigned is lost. Once the project is saved again, projectToDb
// below writes the real team[] column and keeps the 4 legacy columns in sync as a derived courtesy.
const synthesizeTeamFromLegacy = (r: any) => ([
  [r.strategic_lead, 'L1'], [r.project_head, 'L2'], [r.pm, 'L3'], [r.associate, 'L4'],
] as [string, string][]).filter(([name]) => name).map(([name, level]) => ({ name, level }));
...
// desc reuses the existing `root` column (previously "Root Cause", now a free-text issue
// description) rather than adding a new one. tags/remarks are jsonb arrays; pending_status +
// sign_off_* track the assignee-marks-Resolved/Closed -> L1 sign-off flow (see Issues.tsx).
export const issueFromDb = (r: any) => ({ id: r.id, project: r.project, desc: r.root, raisedBy: r.raised_by, assignee: r.assignee, tags: r.tags || [], severity: r.severity, priority: r.priority, due: r.due, status: r.status, pendingStatus: r.pending_status, signOffRequestedBy: r.sign_off_requested_by, signOffRequestedAt: r.sign_off_requested_at, signedOffBy: r.signed_off_by, signedOffAt: r.signed_off_at, addedBy: r.added_by, addedAt: r.added_at, remarks: r.remarks || [] });
...
export const notificationFromDb = (r: any) => ({ ...(r.payload || {}), id: r.id, createdAt: r.created_at });
// Communication messages: same insert-only, jsonb-payload shape as notifications above (id/
// tenant_id/created_at columns, everything else -- projectId, author, body, attachments, mentions,
// parentId for a thread reply -- lives in payload).
export const channelMessageFromDb = (r: any) => ({ ...(r.payload || {}), id: r.id, createdAt: r.created_at });
export async function insertChannelMessage(m: any) {
  const { error } = await supabase.from('channel_messages').insert({ tenant_id: TENANT_ID, id: m.id, payload: m });
  if (error) throw error;
}
...
export const loginLogFromDb = (r: any) => ({ id: r.id, userEmail: r.user_email, userName: r.user_name, at: r.at });
export const activityLogFromDb = (r: any) => ({ id: r.id, userEmail: r.user_email, userName: r.user_name, module: r.module, action: r.action, project: r.project, at: r.at });

// The 4 legacy single-name columns, derived from team[] (L1/L2/L3/L4 respectively) purely so the
// database row stays informative/backward-compatible -- the app itself reads project.team
// everywhere now, these are never read back for logic.
const legacyRolesFromTeam = (team: any[]) => {
  const find = (lvl: string) => (team || []).find((t: any) => t.level === lvl)?.name || null;
  return { strategic_lead: find('L1'), project_head: find('L2'), pm: find('L3'), associate: find('L4') };
};
...
const issueToDb = (i: any) => ({ tenant_id: TENANT_ID, id: i.id, project: i.project, root: i.desc, raised_by: i.raisedBy, assignee: i.assignee, tags: i.tags || [], severity: i.severity, priority: i.priority, due: i.due || null, status: i.status, pending_status: i.pendingStatus || null, sign_off_requested_by: i.signOffRequestedBy || null, sign_off_requested_at: i.signOffRequestedAt || null, signed_off_by: i.signedOffBy || null, signed_off_at: i.signedOffAt || null, added_by: i.addedBy || null, added_at: i.addedAt || null, remarks: i.remarks || [] });
...
async function getBucketDownloadUrl(bucket: string, path: string, expiresIn = 60) {
...
// Communication: file attachments and recorded voice notes posted in a project's channel, same
// private/tenant-scoped bucket pattern as phase-docs. Chat attachments get a longer (1hr, vs the
// usual 60s) signed-URL lifetime -- a voice note sits rendered in the feed waiting to be played,
// possibly long after the message was posted, unlike a document that's downloaded the moment it's
// clicked.
export async function uploadChatFile(id: string, file: File) {
  const r = await uploadToBucket('chat-files', id, file);
  return { id, path: r.path, name: r.name, size: r.size };
}
export const getChatFileDownloadUrl = (path: string) => getBucketDownloadUrl('chat-files', path, 3600);
...
// User Login Log Book — writes only, never read back into live app state (see ActivityLogContext in
// shared.tsx). fetchLoginLogs/fetchActivityLogs are called on demand by Administration.tsx's Login
// Log Book tab, not by the app's normal startup load, since this data is Super Admin-only and could
// grow large over a tenant's lifetime — no reason to make every sign-in pay for it.
export async function insertLoginLog(row: { id: string; userEmail: string; userName?: string }) {
...
export async function fetchLoginLogs(limit = 1000) {
  ...
  if (error) throw error;
  return (data || []).map(loginLogFromDb);
}
export async function insertActivityLog(row: { id: string; userEmail: string; userName?: string; module: string; action: string; project?: string }) {
  const { error } = await supabase.from('activity_logs').insert({ tenant_id: TENANT_ID, id: row.id, user_email: row.userEmail, user_name: row.userName || null, module: row.module, action: row.action, project: row.project || null });
  if (error) throw error;
}
export async function fetchActivityLogs(limit = 1000) {
  const { data, error } = await supabase.from('activity_logs').select('*').order('at', { ascending: false }).limit(limit);
  if (error) throw error;
